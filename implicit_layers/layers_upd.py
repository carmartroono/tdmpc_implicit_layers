...
class ImprovedDEQFunc(nn.Module):
    ...
    def __init__(self, hidden_dim, num_layers=3, dropout=0.1, use_layer_norm=True):
        super().__init__()
        ...
        for i in range(num_layers):
            ...
            layers.append(nn.GELU())  # GELU works better than ReLU for implicit models
...
class DEQ_MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, act=None,
                 f_max_iter=50, b_max_iter=50, f_tol=1e-4, b_tol=1e-6,
                 f_solver='anderson', b_solver='anderson',
                 deq_num_layers=3, dropout=0.1, use_layer_norm=True,
                 log_stats=True, log_every_n_steps=100):
        """
        Args:
            ...
            f_max_iter: Maximum iterations for forward solver (increased from 40)
            b_max_iter: Maximum iterations for backward solver (increased from 40)
            f_tol: Tolerance for forward solver (tighter from 1e-3)
            b_tol: Tolerance for backward solver
            f_solver: Forward solver type
            b_solver: Backward solver type
            deq_num_layers: Number of layers in DEQ function (new)
            dropout: Dropout rate (new)
            use_layer_norm: Whether to use layer normalization (new)
...
class ImprovedDEQConvFunc(nn.Module):
    ...
    def __init__(self, num_channels, num_layers=3, use_batch_norm=True):
        super().__init__()
...
class DEQ_CNN(nn.Module):
    ...
    def __init__(self, obs_shape, num_channels, latent_dim, act=None,
                 f_max_iter=50, b_max_iter=50, f_tol=1e-4, b_tol=1e-6,
                 f_solver='anderson', b_solver='anderson',
                 deq_num_layers=3, use_batch_norm=True,
                 log_stats=True, log_every_n_steps=100):
        """
        Args:
            ...
            f_max_iter: Maximum iterations for forward solver (increased)
            b_max_iter: Maximum iterations for backward solver (increased)
            f_tol: Tolerance for forward solver (tighter)
            b_tol: Tolerance for backward solver
            f_solver: Forward solver type
            b_solver: Backward solver type
    ...
    for k in cfg.obs_shape.keys():
        if k == 'state':
            out[k] = DEQ_MLP(
                ...
                f_max_iter=getattr(cfg, 'deq_f_max_iter', 50),
                b_max_iter=getattr(cfg, 'deq_b_max_iter', 50),
                f_tol=getattr(cfg, 'deq_f_tol', 1e-4),
                ...
                deq_num_layers=getattr(cfg, 'deq_num_layers', 3),
                dropout=getattr(cfg, 'deq_dropout', 0.1),
                use_layer_norm=getattr(cfg, 'deq_use_layer_norm', True),
                # Logging
